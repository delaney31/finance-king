...
import { VoiceFinancialProvider } from "@/components/voice/voice-financial-provider";
import { AccountVoiceSheet } from "@/components/voice/account-voice-sheet";

export function DashboardShell({ children }: { children: React.ReactNode }) {
  return (
    <AskMyCfoProvider>
      <VoiceFinancialProvider>
        <div className="flex h-screen overflow-hidden bg-fk-background">
          ...
          <div className="flex min-h-0 flex-1 flex-col bg-fk-background">
            ...
            <div className="flex min-h-0 flex-1">
              <main className="min-h-0 min-w-0 flex-1 overflow-auto p-4 pb-24 md:p-8 md:pb-8">
                {children}
              </main>
              <AskMyCfoPanel />
            </div>
